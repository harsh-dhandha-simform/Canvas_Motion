...
// eduvideo's own composition: props (scenes/theme/voiceover/audio_url/fps/
// width/height) are supplied per-render via `--props=<job's remotion_plan.json>`
// (see app/render/remotion_adapter.py), not from generated/examples.generated.ts.
// calculateMetadata derives duration/fps/size from those props at render time,
// since every job's video length differs.
const calculateEduVideoMetadata: CalculateMetadataFunction<
  VideoScriptProps
> = async ({ props }) => {
  const durationInFrames = Math.max(
    1,
    props.scenes.reduce((sum, s) => sum + s.duration_frames, 0),
  );
  return {
    durationInFrames,
    fps: props.fps,
...
const EDU_VIDEO_DEFAULT_PROPS: VideoScriptProps = {
  title: "EduVideo placeholder",
  fps: 30,
  width: 1920,
  height: 1080,
  theme: {
  ...
  return (
    <>
      ...
      <Composition
        ...
        width={1920}
        height={1080}
        defaultProps={EDU_VIDEO_DEFAULT_PROPS}
        calculateMetadata={calculateEduVideoMetadata}
      />
    </>
  );
};
